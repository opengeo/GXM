...
Ext.define('GXM.Map', {
    ...
    requires: ['GXM.util.Base', 'GXM.data.LayerStore'],
    extend: 'Ext.Component',
    alias: 'widget.gxm_map',
    isMap: true,
    
    // both 'layers' and 'controls' are not defined via 'config'-object,
    // so that we do not have to decide what happens on call of e.g.
    ...
    initialize: function() {
        ...
        if ( !(this.getMap() instanceof OpenLayers.Map) ) {
            var mapConf = Ext.applyIf(this.getMap() || {}, {
                ...
                numZoomLevels: 24,
                zoom: this.getMapZoom() || undefined,
                center: this.getMapCenter() || undefined
            });
    ...
    /* inherit docs */
   destroy: function(){
       // unbind various listeners to the corresponding OpenLayers.Map-events
        var map = this.getMap();
        if(map && map.events){
            map.events.un({
                "moveend": this.onMoveend,
        ...
        this.element.un('touchstart', 'onTouchStart', this);
        if(this.getGeo()){
            this.un({
        ...
        //if the panel was created with an instance of an OpenLayer.Map
        //then don't touch it. Otherwise, destroy it as well
        if(!this.initialConfig.map || 
                (this.initialConfig.map && !(this.initialConfig.map instanceof OpenLayers.Map))){
            map.destroy();
        }
        
        //always nullify the map reference in this object
        this.setMap(null);
        this.layers = null;
        this.controls = null;
...
//        }
    ...
    /** private: method[updateMapZoom]
     * 
     *  ``Function`` implicitly called whenever :func:`setMapZoom()` is called
     * with a value that differs from the current zoom value
     */
    updateMapZoom: function(mapZoom, oldZoom) {
        mapZoom = parseInt(mapZoom, 10);
        var map = this.getMap();
        if (mapZoom >= 0){
            if (this.isPainted() && map instanceof OpenLayers.Map) {
    ...
    applyMapCenter: function(mapCenter) {
        var ll = null,
            ...
            zoom = false;
        if (!(this.getMap() instanceof OpenLayers.Map)){
            //setting center initially before map is constructed
            zoom = this.getMapZoom() || this.initialConfig.mapZoom || this.initialConfig.zoom;
        }    
   ...
   onRemovelayer: function(olEvt) {
    ...
    updateUseCurrentLocation: function(useCurrentLocation) {
        ...
        // TODO see if we need this in GXM (was copied from Sencha's Map.js) */
